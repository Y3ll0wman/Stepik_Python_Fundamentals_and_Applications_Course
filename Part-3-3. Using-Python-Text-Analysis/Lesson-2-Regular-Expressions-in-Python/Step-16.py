...
def divthree(a: str) -> bool:
    return not (sum(x == '1' for x in a[::2]) - sum(x == '1' for x in a[1::2])) % 3


for number in map(str.strip, sys.stdin):
    if number.count('1') + number.count('0') == len(number) and divthree(number):
        print(number)
